...
- **`VideoModel`** (`backend-python/app/repository/models.py`): Stored in PostgreSQL. Fields: `id`, `title`, `video_url`, `thumbnail` (nullable), `room` (default `"general"`), `uploaded_at`, optional TMDB fields `tmdb_id`, `media_type` (`movie`|`tv`), `season`, `episode` (nullable for legacy YouTube rows). TMDB rows use canonical `video_url` values `tmdb://movie/{id}` or `tmdb://tv/{id}/{season}/{episode}` (not third-party stream URLs). **Schema drift symptom:** after **create or join room**, `Dashboard` + `useWatchParty` immediately call `GET /api/videos?room=...`; if `tmdb_id` (etc.) are missing, Postgres raises `UndefinedColumn` and the room fails on first load—not a WebSocket “create” failure. Apply [`docs/migrations/001_videos_tmdb_columns.sql`](./migrations/001_videos_tmdb_columns.sql) or [`docs/migrations/000_production_bootstrap.sql`](./migrations/000_production_bootstrap.sql) once.
...
| :--- | :--- | :--- | :--- |
| `useWatchParty` | Core WS connection & Room Sync | `urlRoom` (string\|null), `action` (`"join"`\|`"create"`) | On each `room`/`username`/`action` change: clears chat (`messages`), typing, recommendations, playlist state, reconnects WS. **`wsSessionRef`** invalidates stale `WebSocket.onmessage` handlers so join/system lines from a prior room cannot append before React applies `setMessages([])` (fixes duplicate “joined the party” / old chat). Host `sync_state` / `play`/`pause`/`seek` include `provider`. Guests ignore `sync_state` if `msg.provider` ≠ local provider. |
